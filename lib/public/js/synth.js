  ...
  if ( mappingVal !== '4') {
    ...
    var play = setInterval(function () {
      if (i <= len) {
        ...
        mouseEventHandler(currentCoords)
        i = i+1
      } else {
        audio.stop()
        clearInterval(play)
    ...
    setTimeout(function () {
      ...
      audio.play()
      mouseEventHandler(jsonArray)
  }
}

/* function playJSON(coordArray) {
  var jsonArrayX = [coordArray.x];
  var jsonArrayY = [coordArray.y];
  if (mapping === "4") {

    playCoords(coordArray, 8000); return; }

  i = 0;
  len = coordArray.length;
  var totalTime = 1000;
  var intervalTime = totalTime / len;
  if (mapping != "3") { audio.play(); } else { synth.start(); }
  ...
  setTimeout( function() { if (mapping != "3") { audio.stop(); } else { synth.stop(); }
