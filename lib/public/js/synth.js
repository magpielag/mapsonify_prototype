...
const mappings = {
  ...
  '4': function (position) {
    function alterAudio (x, xn, y) {
      var value = { time: scaling.scale((x- xn), 0.0, 8.0, 0, canvas.width), freq: scaling.closest(scaling.logscale(y, 100, 3500, 0, canvas.height)) }
      audio.change('time', value)
    }

    console.log(position.x, position.y)
    // note, position var is a m-d array in the case of mapping four.
    // Find min and max of col 1 (only x values) then scale to 0-8000 (ms).
    var len = position.length
    var timeValues = { min: scaling.scale(arrayTools.min(position.x), 0, 8000, 0, window.innerWidth),
      max: scaling.scale(arrayTools.max(position.x), 0, 8000, 0, window.innerWidth),
      interval: (this.total / len),
      initial: scaling.scale(position.x[0], 0, 8000, 0, window.innerWidth)
    }
    timeValues.total = timeValues.max - timeValues.min
    console.log(timeValues.initial, timeValues.total, timeValues.max, timeValues.min)
    var i = 1

    function executePlayBack() {
      var play = setInterval(function () {
        if (i < len) {
          alterAudio(position.x[i], position.x[i - 1], position.y[i])
          i = i + 1
        }
      }, timeValues.interval)
      ...
      setTimeout(function () {
        console.log('Stop')
        audio.stop()
        position = [] // Return this at the end of the function or use a window.var
        clearInterval(play)
      }, timeValues.total)
    }
    setTimeout(function () {
      console.log('Go')
      audio.play()
      alterAudio(position.x[0], 0, position.y[0])
      executePlayBack()
    }, timeValues.initial)
  }
}
...
xn = xArray[0];
xnMinusOne = xArray[1];
console.log(`xn is ${xn} and xn-1 is ${xnMinusOne}`)
value = { time: scaling.scale((xn - xnMinusOne), 0.0, 8.0, 0, canvas.width),
...
----
    ...
    function playCoords(coordArray, timeValues) {
      i = 1;
      len = coordArray.length;
      console.log(coordArray);
      console.log(timeValues.maxV, timeValues.minV);
      var totalTime = (timeValues.maxV - timeValues.minV);
      var intervalTime = (totalTime / coordArray.length);
      console.log(`Total time is ${totalTime}`);
      console.log(`Array length is ${coordArray.length}, interval time is ${intervalTime}`);
      initialDelay = scaling.scale(coordArray[0][0], 0, 8000, 0, window.innerWidth)
      console.log(initialDelay);

      function executePlay() {
        if (mapping !== '4') {
           var alterAudio = function(n) {
                mouseEventHandler(coordArray[i][0], coordArray[i][1])};
              }
        else { var alterAudio = function(n) {
              mouseEventHandler([coordArray[i][0], coordArray[i-1][0]], coordArray[i][1]); } }

        var play = setInterval( function() {
          console.log(`Play triggered, i equals ${i}, len equals ${len}`);
          if (i < len) {
            alterAudio(i);
            i = i+1;
            console.log(i);
        ...
        setTimeout( function() { audio.stop();
                                 window.timeMouseArray = [];
                                 clearInterval(play); }, totalTime);
      }

      setTimeout(function() { console.log('Finished'); mouseEventHandler([coordArray[0][0], 0], coordArray[0][1]); audio.play(); executePlay();}, initialDelay);
    }
  ...
  switch: function (synthType) {
    switch (synthType) {
      ...
      default:
        ...
        this.play = function () { synth.triggerAttack(synth.frequency.value) }
        break
    }
    synth.connect(synthGain)
  },
  change: function (parameter, value) {
    console.log(context.currentTime, context.currentTime + 0.1)
    switch (parameter) {
      case 'freq':
